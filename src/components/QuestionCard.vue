<template>
  <div class="question-card">
    ...
    <!-- <div class="optionals" v-for="(key, index) in question.options" v-bind:key="index"> -->
      ...
      <div v-for="(answers, index, value) in question.options" :key="answers.id + index">
       <input
          type="radio"
          :value="question.options[index].id"
          v-model="question.id"
        />
        <span> {{answers.id }}|| {{index}} || {{value}} </span>
        <label for="one">{{question.options[index].answer}}</label>
        <br />
        </div>
    </div>
</template>
